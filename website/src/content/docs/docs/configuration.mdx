...
| `publish.series_tag_format` | no | Template every series level substitutes into; must contain `{name}` and `{series}`. Default: `{name}-v{series}`. Use the repository tag keys below for a repository-wide tag. |
...
| `changelog.dependency_body` | no | Minijinja template for one such entry's body. Context: `dependency`, `dependency_version`, `package`. |
...
See [Repository tags](#repository-tags) to configure one shared series tag.
...
```console
$ trellis doctor
error: in gleam.toml: `publish.series-tag-format` was removed; use
...
`[tools.trellis]` carries no unrecognized keys, exiting non-zero on any error.