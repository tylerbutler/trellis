...
rendering is controlled by small minijinja templates. All four formats
(`header-format`, `version-format`, `kind-format`, `change-format`) share the
same context — `name`, `version`, `date`, `tag`, `kind`, `body` — with fields
that don't apply to a given template (e.g. `kind`/`body` in `header-format`)
rendering as empty:
