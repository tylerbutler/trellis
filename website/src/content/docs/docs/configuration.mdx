...
| `publish.tag_format` | no | Template for per-member git tags. Default: `{name}-v{version}`. |
...
| `publish.tag_mode_overrides` | no | Per-package tag modes: a map of mode name to member-path globs. A member matching globs under two modes is an error. |
...
| `changelog.*-format` | no | Minijinja templates for rendered changelog sections: `version_format`, `kind_format`, `change_format`. |
...
| `changelog.dependency_body` | no | Minijinja template for one such entry's body. Context: `dependency`, `dependency_version`, `project`. |
| `doctor.shared_dependencies` | no | How `doctor` treats members disagreeing on a shared external dependency: `warn` (default), `error`, or `off`. |
...
itself. Through v0.7.0 they were kebab-case, and the old spellings still work —
