...
| `exclude.release` | no | The shared package set omitted from changelog, versioning, tagging, publishing, and release CI. |
| `ignore-release` | legacy | Compatibility alias for release exclusions. It remains supported and is combined with `exclude.release`. |
...
The older `ignore-release` array remains supported and is combined with
`exclude.release`. Prefer `exclude.release` in new configuration so every
package-set exception lives under the same task-scoped model.
