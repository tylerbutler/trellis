...
members. In a [manifest adapter](/docs/configuration/#manifest-adapter)
workspace the command is refused outright: the lifecycle ladder tops out at
`git_only`, so there is nothing to publish. It runs, per package and in
dependency order:
