...
kind share the heading rather than producing a second section. A ripple names
no category, so in a workspace using them it files under `uncategorized_label`
alongside anything else that named none.
