...
| `version plan --json` | `trellis.version-plan/1`<br />`{schema, bumped[], fragments-retained}` |
| `version apply --json` | `trellis.version-apply/1`<br />`{schema, bumped[], lockfiles[], adopted[], fragments-retained}` |
...
`fragments-retained` is true only under
...
| `shared-dependency` | members require different versions of the same external dependency |
