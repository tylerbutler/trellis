---
title: Task running
description: trellis run fans a task out across workspace packages, graph-parallel by default, with streamed output and a failure summary.
---
...
Any built-in or custom task can omit packages through a same-named key under
`exclude` — see
...
`trellis exec` runs any command in each package directory, with the same
